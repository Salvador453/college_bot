...
def smart_set_cmd(message):
    ...
    if message.text.strip() == '/set':
        help_text = """🧠 <b>Smart Set - розумне редагування розкладу</b>

Пиши природною мовою, як людині:

<b>Заміна пари:</b>
<code>/set завтра у БЦІГ-25 3 пару замість фізики постав математику в 121</code>
<code>/set в пн БЦІСТ-25 1 пару Фізика 129 Гуленко І.А.</code>

<b>Видалення:</b>
<code>/set прибрати 2 пару в четвер у БЦІГ-25</code>

<b>Додавання:</b>
<code>/set додати в середу БЦІГ-25 5 пару Історія 114 Мелещук</code>
...
⚠️ <b>Всі зміни тимчасові та автоматично скидаються в неділю о 23:00</b>"""
    ...
    action_text = {
        'replace': f"🔄 <b>ЗАМІНА:</b><br>Замість '{parsed['old_subject'] or '...'}' → '{parsed['new_subject'] or '...'}'",
        'add': f"➕ <b>ДОДАВАННЯ:</b><br>{parsed['new_subject'] or '...'}",
        'delete': f"❌ <b>ВИДАЛЕННЯ</b> пари"
    ...
    if parsed['new_subject']:
        ...
        if not test_link:
            link_warning = "<br>⚠️ <b>Увага:</b> Для цього предмету не знайдено посилання Google Meet!<br>Додай його через /setlink<br>"
    
    # ИСПРАВЛЕНИЕ: Используем HTML с <br> для переносов строк
    confirm_text = f"""📋 <b>Перевір дані:</b>{link_warning}<br>
👥 <b>Група:</b> {parsed['group']}<br>
📅 <b>День:</b> {day_name} ({parsed['date_str']})<br>
🔢 <b>Пара:</b> {parsed['pair_num']}<br>
📆 <b>Тиждень:</b> {parsed['week_type']}<br><br>
{action_text}<br>
🏫 <b>Аудиторія:</b> {room_display}<br>
👨‍🏫 <b>Викладач:</b> {teacher_display}<br><br>
⚠️ Це тимчасова заміна (діє до неділі 23:00)<br>
<b>Все вірно?</b>"""
    ...
    bot.reply_to(message, confirm_text, reply_markup=markup, parse_mode="HTML")
