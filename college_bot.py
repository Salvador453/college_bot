...
def extract_pair_num_smart(text):
    """Извлечение номера пары"""
    patterns = [
        r'(\d)\s*-?\s*(?:я|й|ша|та|ю|у)?\s*пар',
        r'(\d)\s*пар',
        r'пар[ауы]?\s*(\d)',
    ]
    
    word_to_num = {
        'первая': 1, 'першу': 1, 'перша': 1, 'первую': 1,
        'другу': 2, 'друга': 2, 'вторая': 2, 'вторую': 2,
        'третью': 3, 'третя': 3, 'третья': 3,
        'четвертую': 4, 'четверта': 4, 'четвертая': 4,
        'пятую': 5, 'пята': 5, 'пятая': 5,
    }
...
def extract_new_subject_info_smart(text):
    """Извлекает новый предмет, аудиторию и учителя"""
    info = {'subject': None, 'room': '', 'teacher': ''}
    
    room_patterns = [
        r'(?:в\s+)?(?:кабинет|аудитория|ауд)[еі]?\s*(\d+[a-zа-я]?)',
        r'(?:room|ауд)\s*(\d+)',
    ...
    stop_words = {'заменить', 'установить', 'добавить', 'поставить', 'на', 'в', 'у', 'пару', 'пара'}
    words = [w for w in remaining.split() if w not in stop_words and len(w) > 2]
...
def parse_smart_set(text, user_id):
    """Основная функция парсинга Smart Set"""
    result = {
        'action': None,
        'group': None,
        'day': None,
    ...
    if any(word in norm_text for word in ['заменить', 'вместо', 'замість']):
...
def smart_set_cmd(message):
    """Smart Set - розумне редагування розкладу"""
    if not is_admin(message):
        return
...
❗ *Перед збереженням покажу підтвердження*"""
...
{action_text}
🏫 Аудиторія: `{parsed['new_room'] or '—'}`
👨‍🏫 Викладач: `{parsed['new_teacher'] or '—'}`
...
def smart_callback_handler(call):
    """Обработка подтверждения Smart Set"""
    ...
    if call.data.startswith("smart_confirm_"):
        ...
        try:
            ...
            if parsed['action'] == 'delete':
                if (group in schedule and day_key in schedule[group] and 
                    week_type in schedule[group][day_key] and 
                    str(pair_num) in schedule[group][day_key][week_type]):
                    del schedule[group][day_key][week_type][str(pair_num)]
                    save_schedule(schedule)
                    result_text = f"✅ Видалено пару {pair_num} ({week_type}) для {group}"
                else:
                    result_text = "❌ Пару не знайдено для видалення"
                ...
                schedule[group].setdefault(day_key, {}).setdefault(week_type, {})
                schedule[group][day_key][week_type][str(pair_num)] = new_pair
...
def admin_help(message):
    ...
    text = (
        "👑 Адмін-команди:\n\n"
        "🧠 *Розумне редагування:*\n"
        "/set - розумна команда для зміни розкладу (пиши природною мовою)\n\n"
        "⚙️ *Ручне редагування:*\n"
        ...
        "Приклади:\n"
        "/set завтра у БЦІГ-25 3 пару замість фізики постав математику\n"
        "/setpair БЦІГ-25 понеділок 1 чисельник Фізика ; 129 ; Гуленко І.А.\n"
        "/setlink БЦІГ-25 Математика https://meet.google.com/xxx  \n"
