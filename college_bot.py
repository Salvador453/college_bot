...
# ================== РАСПИСАНИЯ (ВОЗВРАЩЕНО К ИЗНАЧАЛЬНОМУ ВИДУ) ==================
    ...
    return {
        ...
        "wednesday": {
            ...
            "знаменник": {
                ...
                "5": {"subject": "Захист України",      "room": "242 / 201", "teacher": "Санко / Киянчук"},
            },
        },
        "thursday": {
            "чисельник": {
    ...
    return {
        ...
        "tuesday": {
            ...
            "чисельник": {
                ...
                "4": {"subject": "Всесвітня історія",   "room": "114", "teacher": "Меленчук Ю.Д."},
                "5": {"subject": "Інформатика",         "room": "39",  "teacher": "Короленко / Єреп"},
            },
            "знаменник": {
                "2": {"subject": "Біологія і екологія", "room": "16",  "teacher": "Золотова К.В."},
        ...
        "wednesday": {
            ...
            "чисельник": {
                ...
                "3": {"subject": "Математика",          "room": "121", "teacher": "Приймак О.В."},
                "4": {"subject": "Технології",          "room": "208", "teacher": "Потапова А.О."},
                "5": {"subject": "Захист України",      "room": "242 / 201", "teacher": "Санко / Киянчук"},
            },
            "знаменник": {
                "2": {"subject": "Математика",          "room": "121", "teacher": "Приймак О.В."},
        ...
        "friday": {
            # 1-й пары НЕТ
            "чисельник": {
                "2": {"subject": "Географія",         "room": "123", "teacher": "Бараненко Т.О."},
                "3": {"subject": "Історія України",   "room": "114", "teacher": "Меленчук Ю.Д."},
                "4": {"subject": "Українська література","room": "115", "teacher": "Лосєва К.С."},
            },
            "знаменник": {
                "2": {"subject": "Фізична культура", "room": "с/з №2", "teacher": "Свиридов А.П."},
                "3": {"subject": "Історія України",   "room": "114", "teacher": "Меленчук Ю.Д."},
                "4": {"subject": "Українська література","room": "115", "teacher": "Лосєва К.С."},
            },
        },
...
def load_meet_links():
    """Загружает ссылки для каждой группы из отдельных файлов"""
    # Загружаем ссылки для БЦІГ-25
...
def save_meet_links(links_data):
    """Сохраняет ссылки для каждой группы в отдельные файлы"""
...
# Глобальная переменная для хранения ссылок (словарь словарей)
meet_links = load_meet_links()

def get_meet_link_for_subject(subj: str, group_name: str = None):
    """Получить ссылку для предмета, учитывая группу пользователя"""
    if not subj or not group_name:
        return None
    
    # Получаем ссылки для конкретной группы
    group_links = meet_links.get(group_name, {})
    
    # Сначала проверяем точное совпадение (для Захист України)
    s = subj.strip().lower()
    for key, url in group_links.items():
        if key.strip().lower() == s:
            return url
    
    # Если не нашли точное совпадение, проверяем частичное
    for key, url in group_links.items():
